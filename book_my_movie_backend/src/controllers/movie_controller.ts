...
import { Movie, AddMovieData, UpdateMovie } from "../schema/movie_schema";
...
export const updateMovie = async (req: Request, res: Response) => {
  ...
  const { title, description, image, rating, duration_minutes, release_date } =
...
export const getMovieByTitle = async (req: Request, res: Response) => {
  ...
  try {
    const [rows]: any = await pool.execute(
      `SELECT * FROM movie_tb WHERE title LIKE CONCAT('%', ?, '%')`,
      [title]
    );
