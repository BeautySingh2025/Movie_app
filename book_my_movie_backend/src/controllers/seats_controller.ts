...
export const addSeat = async (
  ...
  if (!hall_id || !seat_number || !price) {
    return res.status(400).json({
      message:
        "hall_id, seat_number and price are required",
    });
  }
  try {
    const sql = `
...
export const getSeatPriceByHallAndSeat = async (
  req: Request,
  res: Response
) => {
  const { hall_id, seat_number } = req.query;
  let seat_no = seat_number;
  // if(typeof seat_number === 'string'){
  //  seat_no  = seat_number.split("\")[0];
    
  // }

  console.log(hall_id,seat_no);

  if (!hall_id || !seat_no) {
    return res.status(400).json({
      message: "hall_id and seat_number are required",
    });
  }
