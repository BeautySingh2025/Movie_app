...
export const createBooking = async (req: Request, res: Response) => {
  const data = req.body as CreateBookingData;
  const {
    user_id,
    movie_id,
    hall_id,
  ...
  try {
    ...
    `,
      [hall_id, movie_id, booking_date, slot_selected],
    );
    const bookedSeats = existing
      .map((b: any) => b.seats_selected.split(","))
      .flat();
    const requestedSeats = seats_selected.split(",");
    const conflict = requestedSeats.filter((s) => bookedSeats.includes(s));
...
export const getAllBookings = async (req: Request, res: Response) => {
  const { page = 1, limit = 10, date } = req.query as any;
  const offset = (page - 1) * limit;
  let sql = "SELECT * FROM booking_tb WHERE 1=1";
  const values: any[] = [];
  if (date) {
...
export const getBookingsByMovie = async (req: Request, res: Response) => {
...
export const getBookingsByHallSlot = async (req: Request, res: Response) => {
  const { hall_id } = req.params;
  const { movie_id, date, slot } = req.query;
  const [rows] = await pool.execute(
    `
      SELECT *
      FROM booking_tb
...
export const updatePaymentStatus = async (req: Request, res: Response) => {
  const { id } = req.params;
  const { payment_status } = req.body as UpdatePaymentData;
  const [result]: any = await pool.execute(
    `
      UPDATE booking_tb
      SET payment_status = ?
...
export const checkSeatAvailability = async (req: Request, res: Response) => {
  try {
    let {  movieId,  hallId, booking_date, slot_selected } =
      req.query;
    const movieIdValue =  movieId;
    const hallIdValue =  hallId;
    if (!movieIdValue) {
      return res
        .status(400)
        .json({ message: "Missing required parameter: movie_id" });
    }
    if (!hallIdValue) {
      return res
        .status(400)
        .json({ message: "Missing required parameter: hall_id" });
    }
    if (!booking_date) {
      return res
        .status(400)
        .json({ message: "Missing required parameter: booking_date" });
    }
    if (!slot_selected) {
      return res
        .status(400)
        .json({ message: "Missing required parameter: slot_selected" });
    }
    ...
    if (isNaN(movieIdNum) || isNaN(hallIdNum)) {
      return res
        .status(400)
        .json({ message: "movie_id and hall_id must be valid numbers" });
    }
    const query = `
      SELECT seats_selected 
      FROM booking_tb 
